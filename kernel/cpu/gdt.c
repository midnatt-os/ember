...
static gdtr_t gdtr = {.limit = sizeof(gdt_entries) - 1,
                      .address = (uint64_t)gdt_entries};

void gdt_load(gdtr_t *gdtr, uint64_t selector_code, uint64_t selector_data);

void gdt_init() { gdt_load(&gdtr, GDT_SEL_CODE_CPL0, GDT_SEL_DATA_CPL0); }

/*void gdt_load_tss(tss_t* tss) {
    uint16_t tss_segment = sizeof(gdt_entries) - 16;

    gdt_sys_entry_t* entry = (gdt_sys_entry_t*) ((uintptr_t) gdt_entries +
tss_segment); entry->entry.access = ACCESS_PRESENT | ACCESS_TYPE_TSS;
