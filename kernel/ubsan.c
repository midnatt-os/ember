...
void __ubsan_handle_load_invalid_value(data_load_invalid_value_t* data, uintptr_t value) {
    logln(LOG_WARN, "UBSAN", "load_invalid_value @ %s:%u:%u {value: %#lx}", data->location.filename, data->location.line, data->location.column, value);
}

void __ubsan_handle_nonnull_arg(data_nonnull_arg_t* data) {
    logln(LOG_WARN, "UBSAN", "handle_nonnull_arg @ %s:%u:%u {arg_index: %i}", data->location.filename, data->location.line, data->location.column, data->arg_index);
}

void __ubsan_handle_nullability_arg(data_nonnull_arg_t* data) {
    logln(LOG_WARN, "UBSAN", "nullability_arg @ %s:%u:%u {arg_index: %i}", data->location.filename, data->location.line, data->location.column, data->arg_index);
}

void __ubsan_handle_nonnull_return_v1(data_only_location_t* data [[maybe_unused]], source_location_t* location) {
    logln(LOG_WARN, "UBSAN", "nonnull_return @ %s:%u:%u", location->filename, location->line, location->column);
}

void __ubsan_handle_nullability_return_v1(data_only_location_t* data [[maybe_unused]], source_location_t* location) {
    logln(LOG_WARN, "UBSAN", "nullability_return @ %s:%u:%u", location->filename, location->line, location->column);
}
...
void __ubsan_handle_negate_overflow(data_location_type_t* data, uintptr_t old) {
    logln(LOG_WARN, "UBSAN", "negate_overflow @ %s:%u:%u {old: %#lx, type: %u-bit %s %s}", data->location.filename, data->location.line, data->location.column, old, info_to_bits(data->type->info), kind_to_type(data->type->kind), data->type->name);
}
...
void __ubsan_handle_type_mismatch_v1(data_type_mismatch_t* data, void* pointer) {
    ...
    if (pointer == nullptr) {
        logln(LOG_WARN, "UBSAN", "type_mismatch @ %s:%u:%u (%s nullptr pointer of type %s)", data->location.filename, data->location.line, data->location.column, kind_strs[data->type_check_kind], data->type->name);
    } else if ((1 << data->alignment) - 1) {
        logln(LOG_WARN, "UBSAN", "type_mismatch @ %s:%u:%u (%s misaligned address %#lx of type %s)", data->location.filename, data->location.line, data->location.column, kind_strs[data->type_check_kind], (uintptr_t) pointer, data->type->name);
    } else {
...
void __ubsan_handle_alignment_assumption(data_alignment_assumption_t* data, void*, void*, void*) {
    logln(LOG_WARN, "UBSAN", "alignment_assumption @ %s:%u:%u", data->location.filename, data->location.line, data->location.column);
}
...
void __ubsan_handle_invalid_builtin(data_invalid_builtin_t* data) {
    logln(LOG_WARN, "UBSAN", "invalid_builtin @ %s:%u:%u", data->location.filename, data->location.line, data->location.column);
}

void __ubsan_handle_pointer_overflow(data_only_location_t* data, void*, void*) {
    logln(LOG_WARN, "UBSAN", "pointer_overflow @ %s:%u:%u", data->location.filename, data->location.line, data->location.column);
}

void __ubsan_handle_builtin_unreachable(data_only_location_t* data) {
    panic("UBSAN", "builtin_unreachable @ %s:%u:%u", data->location.filename, data->location.line, data->location.column);
}

void __ubsan_handle_missing_return(data_only_location_t* data) {
...
void __ubsan_handle_function_type_mismatch(data_function_type_mismatch_t* data, [[maybe_unused]] void* value) {
    logln(LOG_WARN, "UBSAN", "function type mismatch @ %s:%u:%u", data->location.filename, data->location.line, data->location.column);
}
